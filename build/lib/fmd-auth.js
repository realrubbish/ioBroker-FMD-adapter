...
class FmdAuth {
    ...
    async getSalt() {
        try {
            const response = await this.httpClient.post("/salt", { IDT: this.config.username });
     ...
     */
    async deriveKey(saltUrlSafeB64) {
        try {
            // URL-safe Base64 (no padding) -> raw 16 bytes
            const saltBytes = this.base64ToBytes(saltUrlSafeB64);
            if (saltBytes.length !== 16) {
                throw new Error(`Salt length is ${saltBytes.length} bytes, expected 16 (FMD server returns 16-byte salts URL-safe encoded as 22 chars)`);
            }
            ...
            // the server.
            const phcString = await (0, hash_wasm_1.argon2id)({
    ...
    async login(passwordHash) {
        try {
            const response = await this.httpClient.put("/requestAccess", {
     ...
     */
    async getPrivateKey(accessToken) {
        try {
            const response = await this.httpClient.post("/key", { IDT: accessToken });
            if (!response.data?.Data) {
    ...
    async refreshToken() {
        ...
        try {
            ...
            const response = await this.httpClient.post("/requestAccess", {
    ...
    base64ToBytes(base64) {
        ...
        try {
            ...
            throw new Error(`base64ToBytes: cannot decode input (${base64.length} chars, first=${base64.slice(0, 16)}): ${msg}`);
        }
     ...
     * use.
     */
    async deriveKeyFromPrivateKey(privateKey) {
...
exports.FmdAuth = FmdAuth;
//# sourceMappingURL=fmd-auth.js.map